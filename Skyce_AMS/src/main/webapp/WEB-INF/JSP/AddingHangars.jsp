...
<body>
	<div class="wrapper">
		...
		<form:form modelAttribute='hangar' action='addHangar'>
			<div class="main_content">
				<h1 align="center" class="header">Hangar Details</h1>
				<div class="column" style="background-color: white;">
					<div class="form_wrap">

						<label for="hangarID"><b>Hangar ID </b></label>
						<form:input type="hidden" id="hangarNo" path="hangarNo"/>
						<form:input type="text" placeholder="Hangar ID" class="hangarID"
							path="hangarID" required="required" />
						<span class="error">Please Enter Hangar ID</span><br /> <label
							for="Hangar Type"><b>Hangar Type</b></label>
						<form:select placeholder="Hangar Type" class="hangarType"
							path="hangarType" required="required" >
							<form:option value="Plastic">Plastic</form:option>
							<form:option value="Cloth">Cloth</form:option>
							<form:option value="Metal">Metal</form:option>
							<form:option value="Wooden">Wooden</form:option>
							</form:select>
						<span class="error1">Please select Hangar Type </span><br />
					</div>
					
					
					<c:set var="id" scope="session" value="${hangar.hangarNo}" />
                <c:if test="${id!=0}">
	...
	<script type="text/javascript">
		function Validate() {
			...
			var hangarType = document.getElementsByClassName("hangarType");
			var error1 = document.getElementsByClassName("error");
			error1[0].style.display = hangarType[0].value == "" ? "block"
					: "none";
			hangarType[0].style.border = hangarType[0].value == "" ? "1px solid red"
					: "";
			hangarType[0].focus();

			...
			<table width="900" class="collapsetable" border="1" style="background-color: white;">
				...
				<c:forEach var="hangar" items="${hangarList}">
					<tr>
						...
						<c:set var="status" value="${hangar.status}"/>
						<td align="center"><c:if test="${status==0}"><c:set var="sent" value="1"/><c:out value="Available"/>
						</c:if><c:if test="${status==1}"><c:set var="sent" value="0"/><c:out value="Alloted"/></c:if></td>
						<td align="center"><a href="setAvailability?hangarNo=${hangar.hangarNo}&sent=${sent}">Change</a></td>
